        ...
        <View style={sp.overlay}>
          <View style={sp.sheet}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[sp.option, item === value && sp.optionActive]}
                  onPress={() => { onChange(item); setOpen(false); }}
                >
                  <Text style={[sp.optionText, item === value && sp.optionTextActive]}>
                    {item}
                  </Text>
                </TouchableOpacity>
              )}
...
export default function UploadScreen({ navigation }) {
  ...
  const [mediaMode, setMediaMode] = useState(null); // null | 'file' | 'url'
  const [video, setVideo] = useState(null);          // asset object from picker
  const [photo, setPhoto] = useState(null);          // asset object from picker
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Media — file or URL, both optional
      if (video) {
  ...
  const renderStep2 = () => (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <Text style={s.stepSub}>Describe the issue and optionally attach evidence.</Text>
      ...
      {/* Media section */}
      <Text style={f.label}>EVIDENCE <Text style={f.opt}>(optional)</Text></Text>
  ...
  const renderStep3 = () => (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <Field
        ...
        optional
        placeholder="e.g. Near Collectorate, Babina Road"
        value={landmark}
        ...
        {step < TOTAL_STEPS ? (
          ...
          >
            {isUploading
              ...
              : <Text style={s.nextText}>Submit</Text>
            }
          </TouchableOpacity>
        )}
      </View>
